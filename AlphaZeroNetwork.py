...
class AlphaZeroNet( nn.Module ):
    ...
    def __call__( self, x, valueTarget=None, policyTarget=None, policyMask=None ):
        """
        Args:
            x (torch.Tensor) the input tensor. | THE ENTIRE ENCODED BOARD
            valueTarget (torch.Tensor) the value target. | Winner of the game
            policyTarget (torch.Tensor) the policy target. | Index of the encoded target move
        ...
        if self.training:
            ...
            policyMask = policyMask.view( policyMask.shape[0], -1 ) # Flatten all the boards into 1 list
            policy_exp = torch.exp( policy ) 
            policy_exp *= policyMask.type( torch.float32 ) # Convert them into a float
