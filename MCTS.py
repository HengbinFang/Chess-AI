...
class Node:
    ...
    def UCTSelect( self ):
        ...
        for edge in self.edges:
            uct = calcUCT( edge, self.N ) # Calc UCT
            if max_uct < uct: # See if calculated UCT is bigger than current max UCT
                max_uct = uct
                max_edge = edge
    ...
    def getStatisticsString( self ):
        ...
        for edge in edges:
            ...
            string += '|{: ^10}|{:10.4f}|{:10.4f}|{:10.4f}|{:10.4f}|\n'.format(
                str( move ), P, N, Q, UCT )
            data.append({"move": move, "P": P, "N": N, "Q": Q, "UCT": UCT})
...
class Root( Node ):
    ...
    def selectTask( self, board, node_path, edge_path ):
        ...
        Args/Returns:
            board (chess) the root position on input,
